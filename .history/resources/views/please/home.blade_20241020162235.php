<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <form action="{{ route('search.results') }}" method="GET"> 
                ...
                <input type="hidden" id="Hiraganacompany_name" name="Hiraganacompany_name">
                <input type="hidden" id="Kanacompany_name" name="Kananacompany_name">
                <input type="hidden" id="Romajicompany_name" name="Romajicompany_name">
    ...
    <script>
        document.getElementById('company_name').addEventListener('input', function() {
            ...
            const HiraganaCompanyName = WanaKana.toHiragana(companyName);
            const kanaCompanyName = WanaKana.tokana(companyName);
            const RomajiCompanyName = WanaKana.toRomaji(companyName);
            document.getElementById('Hiraganacompany_name').value = HiraganaCompanyName; // 隠しフィールドに設定
            document.getElementById('Kanacompany_name').value = KanaCompanyName; // 隠しフィールドに設定
            document.getElementById('Romajicompany_name').value = RomajiCompanyName; // 隠しフィールドに設定
            console.log(HiraganaCompanyName); // デバッグ用
            console.log(KanaCompanyName); // デバッグ用
            console.log(RomajiCompanyName); // デバッグ用
        });
    </script>
